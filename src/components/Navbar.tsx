...
export const Navbar: React.FC<NavbarProps> = ({ activePage, setActivePage }) => {
  ...
  const currentActive = (activePage || currentPath) as PageView;
  ...
  return (
    <>
            ...
            <div className="main-nav__links">
              {navLinks.map((link) => (
                <button
                  ...
                  className={currentActive === link.id ? 'is-active' : ''}
                >
                  {link.label}
                </button>
              ))}
                    ...
                    <div className="absolute right-0 mt-2 w-60 bg-[#FAF7F2] rounded-xl shadow-xl border border-[#DCD3C1] py-2 z-50 animate-fadeIn">
                      ...
                      <div className="pt-1">
                        <button
                          ...
                          className="w-full text-left px-4 py-2.5 text-xs font-semibold text-[#2A2620] hover:bg-[#F4EFE6] transition-colors flex items-center gap-2"
                        >
                          <svg className="w-4 h-4 text-[#6E6559]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className={`mobile-nav ${mobileOpen ? 'is-open' : ''}`} aria-hidden={!mobileOpen}>
        ...
        {user ? (
          <div className="px-6 py-4 bg-[#EFE8D9] border-b border-[#DCD3C1]">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="w-full text-center text-xs font-semibold text-[#2A2620] py-2 px-3 rounded-lg border border-[#DCD3C1] bg-[#FAF7F2] hover:bg-white transition-colors flex items-center justify-center gap-1.5"
              >
                <svg className="w-3.5 h-3.5 text-[#6E6559]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                  <circle cx="12" cy="7" r="4" />
                </svg>
                My Account
              ...
              <button 
                ...
                className="w-full text-center text-xs font-semibold text-[#8B261E] py-2 px-3 rounded-lg border border-[#8B261E]/30 bg-[#FAF7F2] hover:bg-white transition-colors flex items-center justify-center gap-1.5"
              >
        ...
        <nav className="mobile-nav__links" aria-label="Mobile">
          <button 
            onClick={() => handleNavClick('home')} 
            className={currentActive === 'home' ? 'is-active' : ''}
          >
            Home
          </button>
          {navLinks.map((link) => (
